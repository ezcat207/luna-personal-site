...
export const wayneInsights: WayneInsight[] = [
  ...
  },
  {
    ...
    content: [
      ...
      '用工具的人，每次对话都是一次性的。用完就结束，没有积累，没有系统，没有成长。用团队的人，每次项目都在让系统变好。他的 Prompt 越来越准，他的拆解越来越清晰，他的判断越来越快，他积累的可复用资产越来越多。',
      '一个好的领导者，不在于自己能力有多强，而在于团队能力不断增强。这句话对 AI 时代同样成立。',
      '## 一个公式',
      '我把 AI 领导力浓缩成一个公式：目标 = 团队 + 开会。',
      '目标是你想达成的结果。团队是人 + AI 模型 + 工具的正确配置。开会是拆解、分工、对齐、复盘的协调过程。',
      ...
      '**01 管理：上下文就是你的控制权。** AI 给你什么样的输出，取决于你给它什么样的上下文。什么信息该给？什么信息不该给？什么信息应该长期保存？什么信息只属于当前任务？什么信息要原文保留，什么信息应该压缩成摘要？信息管理能力，直接决定了你能从 AI 那里拿到多高质量的输出。',
      '**02 拆解：不拆解，复杂需求必然失败。** 把一个复杂需求直接丢给 AI，几乎必然得到一个看起来像样但实际不可用的结果。真正的拆解，是把一个复杂需求切成可执行的序列：先让 AI 确认它理解了什么，让它列出不确定的问题，补充信息，然后局部执行，检查，继续。拆解还意味着设计检查点：哪里该停下来让人看？什么时候应该回滚？',
      '**03 判断：设计人机分工，不是盲目外包。** AI 很适合整理资料、生成初稿、提取结构、批量改写。但方向判断、关键取舍、风险承担，仍然要有人负责。真正会用 AI 的人，不会把所有事情都丢给 AI，他会设计人和 AI 的配合方式。',
      '**04 路由：把任务交给对的"人"。** 问题不只是"哪个模型最好"，更重要的是"这一步该由谁来做"。长上下文任务、搜索任务、代码执行任务、结构化输出任务，对应不同的工具选择逻辑。路由能力，是把一个多步骤任务分配给正确工具的调度能力。',
      '**05 诊断：出错不是重试，是追因。** 只会重试，每次都像重新摇骰子。能诊断，系统才会越来越稳定。AI 出错通常有六类根本原因：上下文问题、指令问题、任务拆解问题、模型能力问题、工具调用问题、评估标准问题。当你能快速识别是哪一类，修复就变成定向的，而不是随机的。',
      '**06 沉淀：让每次成功变成下次的起点。** 普通人每次都重新开始，高手越用越强，因为他的经验在不断沉淀。一个好的 Prompt、一套检查清单、一个项目规范、一次有效的人机协作流程，都不应该只停留在某次对话里。沉淀，是把一次成功经验变成可复用资产的能力。',
      '## 把六项能力串起来',
      '这六项能力不是孤立的，它们在每个项目里协同运作：接到一个复杂目标 → 拆解成计划/执行/验证三段 → 给每个任务配置合适的上下文（管理）→ 决定哪些给 AI、哪些人来做（判断）→ 选择合适的模型和工具（路由）→ 执行中出现问题追因修复（诊断）→ 项目完成后提取可复用资产（沉淀）→ 下一个项目从更高的起点开始。',
    ...
    keyTakeaways: [
      ...
      '管理上下文 = 控制 AI 的输出质量；信息的过滤、分层、压缩都是可训练的技能',
      '任务不拆解，复杂需求必然失败；拆解顺序和检查点设计是执行质量的关键',
      '判断哪些给 AI、哪些必须人来做，是领导者最核心的技能，不是可以外包的',
      '诊断 > 重试：AI 出错有六类根本原因，能定位根因才能真正改善系统',
      '沉淀是复利：每次成功经验变成可复用资产，高手越用越强就源于此',
    ],
    relatedResources: [
      {
